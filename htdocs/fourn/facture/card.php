...
$usercanvalidate = $usercancreate; // ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($usercancreate)) || (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->fournisseur->supplier_invoice_advance->validate)));
$usercansend = $usercancreate; // (empty($conf->global->MAIN_USE_ADVANCED_PERMS) || $user->rights->fournisseur->supplier_invoice_advance->send);
	...
	if ($action == 'settransportmode' && ($user->rights->fournisseur->facture->creer || $user->rights->supplier_invoice->creer)) {
		...
		if (GETPOST('type') == FactureFournisseur::TYPE_CREDIT_NOTE) {
			...
			if (!GETPOST('ref_supplier')) {
				setEventMessages($langs->trans('ErrorFieldRequired', $langs->transnoentities('FournFactureRefSupplier')), null, 'errors');
				$action = 'create';
				$_GET['socid'] = $_POST['socid'];
				$error++;
			}
...
if ($action == 'create') {
	...
	// Ref supplier
	print '<tr><td class="fieldrequired">'.$langs->trans('FournFactureRefSupplier').'</td><td><input name="ref_supplier" value="'.(GETPOSTISSET('ref_supplier') ? GETPOST('ref_supplier') : (!empty($objectsrc->ref_supplier) ? $objectsrc->ref_supplier : '')).'" type="text"';
	if (!empty($societe->id) && $societe->id > 0) {
		print ' autofocus';
	}
	print '></td>';
	...
	*/
	...
	if ($id > 0 || !empty($ref)) {
		...
		if ($action == 'clone') {
			...
			$formquestion = array(
				array('type' => 'text', 'name' => 'newsupplierref', 'label' => $langs->trans("FournFactureRefSupplier"), 'value' => $langs->trans("CopyOf").' '.$object->ref_supplier),
		...
		// Ref supplier
		$morehtmlref .= $form->editfieldkey("FournFactureRefSupplier", 'ref_supplier', $object->ref_supplier, $object, $usercancreate, 'string', '', 0, 1);
		$morehtmlref .= $form->editfieldval("FournFactureRefSupplier", 'ref_supplier', $object->ref_supplier, $object, $usercancreate, 'string', '', null, null, '', 1);
		// Thirdparty
