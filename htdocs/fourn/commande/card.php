...
if ($action == 'create' || (empty($action) && empty($id))) {
	...
	if ($user->socid == 0 && $action != 'editline' && $action != 'delete') {
		...
		if (empty($reshook)) {
			...
			//{
			if (((!empty($conf->fournisseur->enabled) && empty($conf->global->MAIN_USE_NEW_SUPPLIERMOD)) || !empty($conf->supplier_invoice->enabled)) && ($object->statut >= 2 && $object->statut != 7)) {  // statut 2 means approved, 7 means canceled
				if ($user->rights->fournisseur->facture->creer || $user->rights->supplier_invoice->creer) {
					print '<a class="butAction" href="'.DOL_URL_ROOT.'/fourn/facture/card.php?action=create&amp;origin='.$object->element.'&amp;originid='.$object->id.'&amp;socid='.$object->socid.'">'.$langs->trans("FournCommButtonCreateBill").'</a>';
				}
			}
			//}
